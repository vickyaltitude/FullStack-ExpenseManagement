...
premiumBtn.addEventListener('click',async (e)=>{
   ...
   try{
    let paymentReq = await fetch(`http://13.233.144.215/premium/buypremium`, {
        headers:{
        "Authorization" : currentUser
    }});
    ...
    console.log(responsereq);
      ...
      let razorUx = {
        ...
        handler : async function(response){
          let upd_trans =  await fetch(`http://13.233.144.215/items/updatetransaction`, {
                method: 'POST', 
                headers: {
                    "Content-Type": "application/json", 
                    "Authorization": currentUser
            ...
            if(upd_trans.ok){
                window.location.href = 'http://13.233.144.215/home/premiumuserhome';
            }
        }
