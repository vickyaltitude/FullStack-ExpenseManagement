...
async function getData(){
    const exp_details = await fetch(`http://43.204.237.132/expenses`, {
        headers:{
...
expForm.addEventListener('submit',async (e)=>{
    ...
    let add_exp = await fetch('http://43.204.237.132/expenses',{
        method: 'POST',
        headers:{
...
async function performAction(e) {
    ...
    if (actionType === 'delete') {
        ...
        let dlt_exp = await fetch('http://43.204.237.132/items/userdelete',{
            method: 'DELETE',
            headers:{
                'Content-Type' : 'application/json',
                "Authorization" : currentUser
        ...
        let edit_exp = await fetch('http://43.204.237.132/items/userpatch',{
            method: 'PATCH',
            headers:{
                'Content-Type' : 'application/json',
                "Authorization" : currentUser
...
premiumBtn.addEventListener('click',async (e)=>{
   ...
   try{
    let paymentReq = await fetch(`http://43.204.237.132/premium/buypremium`, {
        headers:{
        "Authorization" : currentUser
    }});
    ...
    console.log(responsereq);
      ...
      let razorUx = {
        ...
        handler : async function(response){
          let upd_trans =  await fetch(`http://43.204.237.132/items/updatetransaction`, {
                method: 'POST', 
                headers: {
                    "Content-Type": "application/json", 
                    "Authorization": currentUser
            ...
            if(upd_trans.ok){
                window.location.href = 'http://43.204.237.132/home/premiumuserhome';
            }
        }
